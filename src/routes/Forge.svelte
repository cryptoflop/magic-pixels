...
<div class="grid grid-cols-[min-content,min-content] grid-rows-[1fr,min-content] gap-x-4 gap-y-2 m-auto">
  ...
  >
    ...
    <!-- <svg
      ...
      height={PLATE_SIZE * 24}
      width={PLATE_SIZE * 24}
      viewBox="0 0 {PLATE_SIZE} {PLATE_SIZE}"
    >
      {#each placedPixels as pxl, i (i)}
        {#if pxl}
          <rect width="1" height="1" x={i % PLATE_SIZE} y={Math.ceil((i + 1) / PLATE_SIZE) - 1} fill={pixelColor(pxl[0])}>
            <animate
              attributeName="fill"
              dur="{pxl.length + 1}s"
              repeatCount="indefinite"
              begin="{(delays[i] || 0) / 100}s"
              values={pxl
                .concat([pxl[0]])
                .map((p) => pixelColor(p))
                .join(";")}
