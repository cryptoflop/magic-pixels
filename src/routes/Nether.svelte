...
<div class="grid m-auto">
  <div
    ...
    bind:this={container}
    style="grid-template-columns: repeat({Math.floor(Math.sqrt(batches * BATCH_SIZE))},1fr);"
  >
    {#each pixels as pxl, i (i)}
      <div class="border-2">
        {#if pxl[0] == EMPTY}
          <div class="h-6 w-6 text-center text-lg align-baseline select-none">?</div>
        {:else}
  ...
  <div class="mt-4 grid">
    ...
    {:else}
      <button on:click={conjure} class="select-none text-lg button">Conjure pixels</button>
    {/if}
  </div>
</div>

<div class={`grid mt-auto ${conjuring && "pointer-events-none select-none opacity-40"}`}>
  <div class="flex mx-auto">
    <div>{ethPrice.toFixed(3)}eth</div>
    <span>&ensp;=&ensp;</span>
    <span use:tooltip={`1 eth = $${$usd}`}>${(ethPrice * $usd).toFixed(2)}</span>
    <span>&ensp;=&ensp;</span>
    <span>{batches * BATCH_SIZE} pixels</span>
  </div>
</div>

<div class={`flex mx-auto space-x-4 mb-4 ${conjuring && "pointer-events-none select-none opacity-40"}`}>
  <input class="block my-auto ml-auto" type="range" min="1" max="10" step="1" bind:value={batches} />
