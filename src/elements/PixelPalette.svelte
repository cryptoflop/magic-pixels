...
<div class="border-2 {className}">
  ...
  <div class="flex">
    <button class="mr-auto ml-2 mt-[4px] mb-[4px] button" on:click={() => (filterOpen = true)}>Filter</button>
    <div class="text-xs mt-2 mr-2">{filterLength} active</div>
  </div>

  <div class="relative h-[352px]">
    {#if availablePixels.length == 0}
      <div class="absolute text-center text-xs opacity-60 pl-2 left-0 right-0">No pixels</div>
    {/if}

    <div style="min-width: {cols * 24 + 8}px; height: 352px;">
      <VirtualList class="overflow-y-scroll" classRow="flex" itemHeight={24} items={pixelRows} let:item>
        {#each item as pxl}
          <div class="relative" on:mousedown={(e) => dispatch("mousedown", { pxl, ev: e })}>
            <Pixel class="h-6 w-6" pixel={pxl} />
            <div class="absolute inset-0 hover:bg-white/30 select-none {classNamePixel}" use:tooltip={fullPixelName(pxl)} />
          </div>
        {/each}
      </VirtualList>
    </div>
