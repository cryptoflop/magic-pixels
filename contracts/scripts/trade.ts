...
export async function openTrade(acc: Awaited<ReturnType<typeof viem.getWalletClients>>[0], pxlsAddress: `0x${string}`, receiver: `0x${string}`) {
	...
	const conjureTx = await pxls.write.conjure([2n], { value: parseEther("0.05") })
	...
	const tradeTx = await ah.write.openTrade([receiver, bytesToPixelBytes(conjured.args.pixels), 0n, true])
