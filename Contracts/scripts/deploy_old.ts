...
export async function deploy () {
  ...
  const pxls = await (await ethers.getContractFactory('MagicPixels', creatooor)).deploy({ gasLimit: 5000000 })
  await pxls.deployed()
  const plts = await (await ethers.getContractFactory('MagicPlates', creatooor)).deploy({ gasLimit: 5000000 })
  await plts.deployed()

  console.log('deployed')

  await pxls.setMagicPlates(plts.address)
  await plts.setMagicPixels(pxls.address)
  ...
  const tx = await pxls.conjure(10)
  const rc = await tx.wait()
  console.log(rc.events![0].args![1])
  ...
  console.log(await plts.tokenURI(0))
